...
export async function uploadChunks(db,req,body,session,limits){
  ...
  if(req==='/api/upload/start'){
    ...
    const used=Number((await db.prepare('SELECT COALESCE(SUM(size),0) AS used FROM files WHERE owner=?').get(globalThis.__oriUser?.()||1)).used),pending=Number((await db.prepare('SELECT COALESCE(SUM(size),0) AS used FROM uploads').get()).used);
