...
export const ITEM_TYPES = ['rock', 'todo', 'issue', 'note', 'resource', 'event'];
...
export function attention(i, now = new Date()) {
  const age=Math.max(0,Math.floor((+now-new Date(i.created_at))/DAY));
  return {age,level:age>=14?'critical':age>=7?'attention':'normal',overdue:!complete(i)&&!!itemDate(i)&&itemDate(i)<today(now),snoozed:!!i.review_snooze_until&&new Date(i.review_snooze_until)>now};
}
...
export function normalize(input, now = new Date()) {
  ...
  s.schema=2; s.revision=Number(s.revision)||0;s.projects||=[];s.orders||={};s.focusThree||=[];s.audit||=[];s.outputs||=[];s.captures||=[];s.connections||=[];
...
export function validateState(next, previous, now = new Date()) {
  ...
  for(const p of next.projects){if(!p.name?.trim()||!next.worlds.some(w=>w.id===p.world))fail('Each Project requires a name and one World.');if(p.rock&&!next.items.some(i=>i.id===p.rock&&i.type==='rock'&&i.world===p.world))fail('A Project’s primary Rock must belong to its World.');}
  for(const i of next.items){
    ...
    if(i.due_at&&!validDate(i.due_at))fail('Enter a valid due date.');
