...
if(!(await db.prepare('SELECT id FROM profile WHERE id=1').get())){const salt=randomBytes(16).toString('hex');(await db.prepare('INSERT INTO profile VALUES(1,?,?,?) ON CONFLICT(id) DO NOTHING').run(salt,scryptSync(process.env.TEST_PASSWORD||'vanta',salt,64).toString('hex'),JSON.stringify(seed)));}
...
const mime={'.html':'text/html','.js':'text/javascript','.css':'text/css','.png':'image/png','.webp':'image/webp','.svg':'image/svg+xml','.webmanifest':'application/manifest+json','.mp3':'audio/mpeg','.wav':'audio/wav','.m4a':'audio/mp4'};
