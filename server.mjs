...
const userScope=new AsyncLocalStorage();const uid=()=>userScope.getStore()||1;globalThis.__oriUser=uid;
...
if(!(await db.prepare('SELECT id FROM profile WHERE id=1').get())){const salt=randomBytes(16).toString('hex');(await db.prepare('INSERT INTO profile (id,salt,hash,data) VALUES(1,?,?,?) ON CONFLICT(id) DO NOTHING').run(salt,scryptSync(process.env.AMECHI_PASSWORD||process.env.TEST_PASSWORD||'vanta',salt,64).toString('hex'),JSON.stringify(normalize(seed))));}
// Multi-account: per-user rows. Existing single-user data stays on profile id 1.
...
const ACCOUNTS=[{id:1,email:'amechi@addcolormedia.com',name:'Amechi',password:process.env.AMECHI_PASSWORD||process.env.TEST_PASSWORD||'vanta',seed:'amechi'},{id:2,email:'hello@iamphoenixwhite.com',name:'Phoenix',password:process.env.PHOENIX_PASSWORD||'phoenix',seed:'phoenix'}];
for(const a of ACCOUNTS){const existing=await db.prepare('SELECT id,data FROM profile WHERE id=?').get(a.id);
  if(existing){if(process.env.ORI_TEST)continue;// Clean-break reseed: when the bundled seed carries a newer seed_version, replace the workspace once. Password, files and Knowledge chats are kept.
  ...
  try{const files=JSON.parse(await readFile(resolve(import.meta.dirname,'seeds',a.seed+'-files.json'),'utf8'));for(const f of files){const buf=await readFile(resolve(import.meta.dirname,'seeds',f.path));await db.prepare('INSERT INTO files (id,name,type,size,uploaded_at,content,owner) VALUES(?,?,?,?,?,?,?) ON CONFLICT(id) DO NOTHING').run(f.id,f.name,f.type,buf.length,f.uploaded_at,buf,a.id)}}catch{}}
// Passwords come from Vercel environment settings when present, applied on every start.
for(const [pid,pw] of [[1,process.env.AMECHI_PASSWORD],[2,process.env.PHOENIX_PASSWORD]]){if(!pw)continue;const row=await db.prepare('SELECT salt,hash FROM profile WHERE id=?').get(pid);if(row&&!timingSafeEqual(scryptSync(pw,row.salt,64),Buffer.from(row.hash,'hex'))){const salt=randomBytes(16).toString('hex');await db.prepare('UPDATE profile SET salt=?,hash=? WHERE id=?').run(salt,scryptSync(pw,salt,64).toString('hex'),pid)}}
const FILE_LIMIT=20*1024*1024, ACCOUNT_LIMIT=500*1024*1024;
...
async function rawState(){return JSON.parse((await db.prepare('SELECT data FROM profile WHERE id=?').get(uid())).data);}
async function storeState(s,previous){const data=JSON.stringify(s);if(previous){const result=await db.prepare('UPDATE profile SET data=? WHERE id=? AND data=?').run(data,uid(),JSON.stringify(previous));if(!Number(result.changes))throw conflict();}else await db.prepare('UPDATE profile SET data=? WHERE id=?').run(data,uid());return s;}
...
const legacy=await rawState();if(!legacy.schema||legacy.schema<2){if(!process.env.VERCEL){await mkdir(resolve(import.meta.dirname,'.data/backups'),{recursive:true});await writeFile(resolve(import.meta.dirname,'.data/backups/migration-v1-'+Date.now()+'.json'),JSON.stringify(legacy));}for(const i of legacy.items){if(i.file?.startsWith('data:')){const [,type,data]=i.file.match(/^data:([^;]+);base64,(.*)$/s)||[];if(data){const id=randomBytes(16).toString('hex'),buf=Buffer.from(data,'base64');(await db.prepare('INSERT INTO files (id,name,type,size,uploaded_at,content,owner) VALUES(?,?,?,?,?,?,?)').run(id,i.filename||i.title,type,buf.length,i.created?i.created+'T12:00:00Z':new Date().toISOString(),buf,uid()));i.file_id=id;i.filename||=i.title;i.file_size=buf.length;i.uploaded_at=i.created?i.created+'T12:00:00Z':new Date().toISOString();delete i.file;}}if(i.title?.startsWith('Checkpoint:'))i.description||=i.title; }await storeState(normalize(legacy));}
...
app.use(async(req,res)=>{try{
  ...
  if(url.pathname.startsWith('/api/')){
    ...
    if(url.pathname==='/api/login'&&req.method==='POST'){
      ...
      const email=String(body.email||'').trim().toLowerCase();const p=(await db.prepare('SELECT * FROM profile WHERE LOWER(email)=?').get(email));const pass=String(body.password||'');const valid=pass;
      if(!p||!timingSafeEqual(scryptSync(valid,p.salt,64),Buffer.from(p.hash,'hex'))){failures.set(ip,{count:(f?.count||0)+1,time:Date.now()});return send(res,401,{error:'Email or password is incorrect.'})}
      failures.delete(ip);const token=randomBytes(32).toString('hex');(await db.prepare('INSERT INTO sessions (token,expires,profile_id) VALUES(?,?,?)').run(token,Date.now()+604800000,p.id));res.setHeader('Set-Cookie',`ori=${token}; HttpOnly; SameSite=Strict; Path=/; Max-Age=604800${(process.env.SECURE_COOKIE==='1'||process.env.VERCEL)?'; Secure':''}`);return send(res,200,{ok:true});
    }
    let token=req.headers.cookie?.match(/(?:^|; )ori=([a-f0-9]+)/)?.[1];if(url.pathname==='/api/google/callback'){const nonce=url.searchParams.get('state'),oauthCookie=req.headers.cookie?.match(/(?:^|; )ori_oauth=([a-f0-9]+)/)?.[1];if(nonce&&nonce===oauthCookie)token=(await db.prepare('SELECT session FROM oauth_states WHERE nonce=? AND expires>?').get(nonce,Date.now()))?.session;}const sessionRow=token&&(await db.prepare('SELECT token,profile_id FROM sessions WHERE token=? AND expires>?').get(token,Date.now()));if(!sessionRow)return send(res,401,{error:'Please sign in.'});userScope.enterWith(Number(sessionRow.profile_id)||1);
    if(url.pathname==='/api/logout'&&req.method==='POST'){(await db.prepare('DELETE FROM sessions WHERE token=?').run(token));res.setHeader('Set-Cookie','ori=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0');return send(res,200,{ok:true})}
    ...
    if(url.pathname==='/api/state'&&req.method==='PUT'){
      ...
      const next=validateState(body,previous);for(const i of next.items.filter(i=>i.file_id)){const f=(await db.prepare('SELECT uploaded_at,size,name FROM files WHERE owner=? AND id=?').get(uid(),i.file_id));if(!f)return send(res,400,{error:'Uploaded file is missing.'});i.uploaded_at=f.uploaded_at;i.file_size=f.size;i.filename=f.name;}
    ...
    if(url.pathname==='/api/config'){
      ...
      return send(res,200,{fileLimit:FILE_LIMIT,accountLimit:ACCOUNT_LIMIT,used:(await db.prepare('SELECT COALESCE(SUM(size),0) AS used FROM files WHERE owner=?').get(uid())).used,ai:!!(process.env.ANTHROPIC_API_KEY&&process.env.ANTHROPIC_MODEL),google:await google.status(),tracks:tracks.filter(n=>['.mp3','.wav','.m4a'].includes(extname(n).toLowerCase())).map(n=>({name:basename(n,extname(n)),url:'/music/'+encodeURIComponent(n)}))});
    }
    ...
    if(url.pathname==='/api/upload'&&req.method==='POST'){
      ...
      const used=Number((await db.prepare('SELECT COALESCE(SUM(size),0) AS used FROM files WHERE owner=?').get(uid())).used);if(used+bytes.length>ACCOUNT_LIMIT)return send(res,413,{error:'The account’s 200 MB storage quota is full.'});
      const id=randomBytes(16).toString('hex'),at=new Date().toISOString();(await db.prepare('INSERT INTO files (id,name,type,size,uploaded_at,content,owner) VALUES(?,?,?,?,?,?,?)').run(id,basename(body.name),extensions[ext],bytes.length,at,bytes,uid()));
    ...
    if(url.pathname.startsWith('/api/files/')&&req.method==='GET'){
      const f=(await db.prepare('SELECT * FROM files WHERE id=? AND owner=?').get(url.pathname.split('/').at(-1),uid()));if(!f)return send(res,404,{error:'File not found.'});res.writeHead(200,{'Content-Type':f.type,'Content-Disposition':`attachment; filename*=UTF-8''${encodeURIComponent(f.name)}`,'X-Content-Type-Options':'nosniff','Cache-Control':'private, no-store'});return res.end(Buffer.from(f.content));
    }
    if(url.pathname==='/api/backup'&&req.method==='GET'){
      const backup={format:'CNFDNT OS portable backup',version:3,knowledgeThreads:(await db.prepare('SELECT * FROM knowledge_threads WHERE owner=?').all(uid())).map(t=>({...t,scope:JSON.parse(t.scope),messages:JSON.parse(t.messages)})),exported_at:new Date().toISOString(),state:await readState(),files:(await db.prepare('SELECT * FROM files WHERE owner=?').all(uid())).map(f=>({...f,content:Buffer.from(f.content).toString('base64')}))};res.setHeader('Content-Disposition','attachment; filename="ori-portable-backup.json"');return send(res,200,backup);
    }
    if(url.pathname==='/api/knowledge'&&req.method==='POST'){
      ...
      const previous=body.threadId?(await db.prepare('SELECT * FROM knowledge_threads WHERE id=? AND owner=?').get(String(body.threadId),uid())):null;
      ...
      (await db.prepare('INSERT INTO knowledge_threads (id,scope,messages,owner) VALUES(?,?,?,?) ON CONFLICT(id) DO UPDATE SET scope=excluded.scope,messages=excluded.messages').run(id,JSON.stringify(scope),JSON.stringify([...history,message].slice(-30)),uid()));
    ...
    if(url.pathname==='/api/google/auth'&&req.method==='POST'){const result=await google.auth(token),nonce=new URL(result.url).searchParams.get('state');res.setHeader('Set-Cookie',`ori_oauth=${nonce}; HttpOnly; SameSite=Lax; Path=/api/google; Max-Age=600${(process.env.SECURE_COOKIE==='1'||process.env.VERCEL)?'; Secure':''}`);return send(res,200,result);}
    if(url.pathname==='/api/google/callback'){await google.callback(url.searchParams.get('code'),url.searchParams.get('state'),token);res.setHeader('Set-Cookie','ori_oauth=; HttpOnly; SameSite=Lax; Path=/api/google; Max-Age=0');res.writeHead(302,{Location:'/?google=connected'});return res.end();}
...
if(!process.env.VERCEL)app.listen(Number(process.env.PORT||4310),process.env.HOST||'127.0.0.1',()=>console.log(`Orí http://localhost:${process.env.PORT||4310}`));
